...
async def add_connection(connection: QueueConnectionConfigTypes):
    JsonFilesService.insert_json(connection.name, JsonType.CONNECTION, connection.model_dump_json())
    return {"message": "Connection added"}

@router.get("/connection")
async def get_connection()->list[str]:
    return JsonFilesService.get_jsons_name_by_type(JsonType.CONNECTION)

@router.get("/connection/{name}")
async def get_connection(name:str)->dict:
    return JsonFilesService.get_json_by_name_and_type(name, JsonType.CONNECTION)

@router.delete("/connection/{name}")
async def delete_connection(name: str):
    count = JsonFilesService.delete_json_by_name_and_type(name, JsonType.CONNECTION)
